...
- 日記にスマホの写真を1枚添付できます。
- 手書きメモを保存できます。
- 日記本文、添付写真、手書きメモから「今日の1枚」PNG画像を自動作成できます。
...
- 日記データ、日記の添付写真、手書きメモ、Wishデータ、達成写真、感想、今日の1枚画像は、スマホやブラウザ内のIndexedDBに保存されます。
...
1. `今日` 画面を開きます。
2. `写真を添付` でスマホの写真を選びます。
3. プレビューを確認します。
4. `日記を保存` を押します。

写真は保存前にブラウザ内のCanvasで横幅1200px程度へ縮小され、日付ごとの日記に紐づいてIndexedDBに保存されます。写真を外したい場合は `写真を削除` を押してから `日記を保存` してください。

## 今日の1枚を作る

1. `今日` 画面で日記を書きます。
2. 必要なら写真を添付し、手書きメモを書いて `手書きを保存` を押します。
3. `今日の1枚を作る` を押します。
4. 日付、タイトル、本文、気分、タグ、添付写真、手書きメモをまとめた縦長PNGカードが作られます。
5. `PNGを保存` を押すとスマホに画像として保存できます。

この機能はAI画像生成ではありません。スマホ内のHTML/CSS/Canvasだけで、日記カード画像を自動生成します。生成した画像は、その日の日記に紐づいてIndexedDBに保存されます。GitHubには日記データや写真は保存されません。
